...
// An FMalloc::Free call site generated by UE 4.27. The leading instruction is
// `mov r8, [rip + displacement]`: every matching call site must therefore
// agree on the address of the global GMalloc pointer.
inline constexpr auto GMallocReferencePattern = makePattern(
    "48 85 C9 74 2E 53 48 83 EC 20 48 8B D9 48 8B ?? "
    "?? ?? ?? ?? 48 85 C9");
...
RuntimeSymbolFailure allocatorFailure(ResolutionStatus status) noexcept {
    switch (status) {
    case ResolutionStatus::NotFound:
        return RuntimeSymbolFailure::GMallocNotFound;
    case ResolutionStatus::Ambiguous:
        return RuntimeSymbolFailure::GMallocAmbiguous;
    default:
        return RuntimeSymbolFailure::GMallocInvalid;
    }
}
...
RuntimeSymbolResolution resolveRuntimeSymbols(const PeImageView& image) {
    ...
    const auto allocator = resolveRipRelativeConsensus(
        text->Bytes,
        text->Rva,
        PatternView(GMallocReferencePattern),
        16,
        20,
        0,
        image.imageSize());
    if (allocator.Status != ResolutionStatus::Resolved)
        return {allocatorFailure(allocator.Status), {}};

    const auto allocatorData = image.sectionContaining(allocator.Rva, sizeof(void*));
    if (!allocatorData || std::string_view(allocatorData->Name.data()) != ".data")
        return {RuntimeSymbolFailure::GMallocOutsideData, {}};
...
std::string_view runtimeSymbolFailureName(RuntimeSymbolFailure failure) noexcept {
    ...
    case RuntimeSymbolFailure::GMallocNotFound: return "GMalloc references missing";
    case RuntimeSymbolFailure::GMallocAmbiguous: return "GMalloc target ambiguous";
    case RuntimeSymbolFailure::GMallocInvalid: return "GMalloc reference invalid";
    case RuntimeSymbolFailure::GMallocOutsideData: return "GMalloc target outside .data";
    }
