...
The dedicated server listens on `0.0.0.0:50000/TCP` by default. Unreal owns
`50000/UDP`; the two transports can use the same numeric port. A bounded JSON
...
client stores one `127.0.0.1:50000` endpoint for both channels.
...
no administrative operations. Administration protocol version 6 uses a
